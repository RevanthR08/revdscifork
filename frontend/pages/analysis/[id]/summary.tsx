  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (summaryData) {
        setSummary({
          ...summaryData,
          content_markdown: summaryData.ai_summary_report || summaryData.content_markdown || summaryData.executive_briefing,
          sections: summaryData.sections || {
            executive_summary: summaryData.executive_briefing,
            attack_narrative: "See detailed report below.",
            affected_assets: "Refer to Key Findings section.",
      ...
      setFindings((findingsData.findings || []).map((f: any) => ({
        ...
        details: { count: f.count, description: f.description }
      })))
      ...
      setChains((chainsData.chains || []).map((ch: any, idx: number) => ({
        ...
        chain_index: ch.chain_index || idx + 1,
        title: ch.title,
  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      if (data) {
        setSummary({
          ...data,
          content_markdown: data.ai_summary_report || data.content_markdown || data.executive_briefing,
                        ...
                        <div className="px-6 pb-6 border-t border-zinc-800 pt-6">
                          <div className="prose prose-sm prose-invert max-w-none">
                            <ReactMarkdown 
                              ...
                              components={{
                                h1: ({node, ...props}) => (
                                  <h1 className="text-2xl font-black text-white tracking-wide uppercase mb-2 mt-2" {...props} />
                                ),
                                h2: ({node, ...props}) => (
                                  <div className="flex items-center gap-3 mt-10 mb-4">
                                    <div className="w-1 h-5 bg-violet-500 rounded-full flex-shrink-0" />
                                    <h2 className="text-[13px] font-black text-white uppercase tracking-[0.12em]" {...props} />
                                  </div>
                                ),
                                h3: ({node, ...props}) => <h3 className="text-md font-bold text-zinc-200 mb-3 mt-6 uppercase tracking-wide" {...props} />,
                                p: ({node, ...props}) => <p className="text-zinc-400 leading-relaxed mb-4" {...props} />,
                                table: ({node, ...props}) => (
                                  <div className="my-6 overflow-hidden border border-zinc-700 rounded-md">
                                    <table className="w-full border-collapse text-left text-sm" {...props} />
                                  </div>
                                ),
                                thead: ({node, ...props}) => <thead className="bg-zinc-800/80 border-b border-zinc-700" {...props} />,
                                th: ({node, ...props}) => <th className="p-3 text-zinc-100 font-bold uppercase text-[10px] tracking-wider border-r border-zinc-700 last:border-r-0" {...props} />,
                                td: ({node, ...props}) => <td className="p-3 text-zinc-400 border-r border-zinc-800 last:border-r-0 border-b border-zinc-800" {...props} />,
                                tr: ({node, ...props}) => <tr className="hover:bg-zinc-800/20 last:border-b-0" {...props} />,
                                hr: ({node, ...props}) => <hr className="border-zinc-800 my-8" {...props} />,
                                li: ({node, ...props}) => <li className="text-zinc-400 my-1 ml-4 list-disc" {...props} />,
